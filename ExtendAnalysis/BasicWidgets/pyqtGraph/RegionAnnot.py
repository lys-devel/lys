...
class RegionAnnotCanvas(RectAnnotationSettingCanvas):
    def __init__(self,dpi):
        super().__init__(dpi)
        self._registerType('region')
    @saveCanvas
    def addRegion(self,pos,size,axis=Axis.BottomLeft,appearance=None,id=None):
        roi=pg.LinearRegionItem()
        return self.addAnnotation('region','region',roi,appearance=appearance,id=id)
    def addCallback(self,indexes,callback):
        list=self.getAnnotations('region',indexes)
        for l in list:
...
class RegionAnnotationSettingCanvas(RegionAnnotCanvas):
    pass
